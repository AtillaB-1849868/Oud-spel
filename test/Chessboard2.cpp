...
bool Chessboard2::setPiece(std::string startPos, std::string endPos, Color turn) {
	...
	if (typeStart == Type::out || typeEnd == Type::out) //if out of border
		return false;
	if (start->getColor() == turn) //if moving piece of valid color						//if type fixed -> getColor == turn + type
		if (typeEnd == Type::empty)
			valid = m_board[rowStart][colStart]->checkMove(typeEnd, Color::black, rowStart, colStart, rowEnd, colEnd);
		else
			valid = m_board[rowStart][colStart]->checkMove(typeEnd, end->getColor(), rowStart, colStart, rowEnd, colEnd); //check validity
...
bool Chessboard2::AISetPiece() {
	...
	if (start->getColor() == Color::black) { //if piece is Black pawn (moet veranderen in toekomst)
		...
		if (typeLeftUnder != Type::out && typeLeftUnder != Type::empty) { //if a piece positioned leftUnder
			valid = m_board[rowStart][colStart]->checkMove(typeLeftUnder, m_board[rowStart+1][colStart-1]->getColor(), rowStart, colStart, rowStart + 1, colStart + 1); //check if move posible
			if (valid) {
				if (start->getType() == Type::pawn)
					m_board[rowStart][colStart]->setHasMadeMove(true);
		...
		if (typeRightUnder != Type::out && typeRightUnder != Type::empty) { //if a piece positioned rightUnder
			valid = m_board[rowStart][colStart]->checkMove(typeRightUnder, m_board[rowStart+1][colStart+1]->getColor(), rowStart, colStart, rowStart + 1, colStart - 1); //check if move posible
			if (valid) {
				if (start->getType() == Type::pawn)
					m_board[rowStart][colStart]->setHasMadeMove(true);
				m_board[rowStart + 1][colStart + 1] = m_board[rowStart][colStart];
		...
		if (m_board[rowStart][colStart]->getHasMadeMove() == false && m_board[rowStart][colStart]->getType() == Type::pawn) { //if the pawn has not moved yet
			...
			valid = m_board[rowStart][colStart]->checkMove(typeEnd, m_board[rowStart+move][colStart]->getColor(), rowStart, colStart, rowStart + move, colStart); //check if move posible
			if (valid) {
				m_board[rowStart][colStart]->setHasMadeMove(true);
		...
		valid = m_board[rowStart][colStart]->checkMove(typeEnd, m_board[rowStart+1][colStart]->getColor(), rowStart, colStart, rowStart + 1, colStart); //otherwise just move one down
		if (valid) {
			m_board[rowStart][colStart]->setHasMadeMove(true);
...
bool Chessboard2::checkVictory() {
	for (int i = 0; i < BOARDSIZE; i++) {
		if (m_board[0][i] != nullptr) { //a pawn is at blackside
			if (m_board[0][i]->getColor() == Color::white) {
				printBoard();
				return true;
			}
		}
		else if (m_board[7][i] != nullptr) {
			if (m_board[7][i]->getColor() == Color::black) {  //if a pawn is at white side
				printBoard();
				return true;
			}
		}
